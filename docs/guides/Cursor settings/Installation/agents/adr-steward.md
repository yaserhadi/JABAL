...
- If owner is not provided by user, set Owner = KSS Steward - YH.
...
## Default Owner
ADR default Owner = KSS Steward - YH. Override only if user explicitly provides a different owner.
...
Generated-by: adr-steward (tool metadata, not authority)
