...
Apply these in your Cursor command files (e.g. under `C:\Users\YH\.cursor\commands\`):
...
- **boot** (e.g. `boot.md`): Check policy at `.cursor/DOC_POLICY.yaml`; read `.cursor/memory/AI_ENTRY.md` (mandatory order: DOC_POLICY, then PROJECT_MANIFEST, STATE, HANDOFF, GOALS), then `.cursor/memory/PROJECT_MANIFEST.md`, `.cursor/memory/STATE.yaml`, `.cursor/memory/HANDOFF.md`. The agent must read PROJECT_MANIFEST to receive the project brain. On-demand sources include `docs/architecture/ADR/README.md` for architecture decisions.
...
**Single-memory-path fix (done):** Commands updated; content migrated to `.cursor/memory/`; root `memory/` deprecated (README only). **You should run `/session-end` once and confirm only `.cursor/memory/HANDOFF.md` is updated.**
