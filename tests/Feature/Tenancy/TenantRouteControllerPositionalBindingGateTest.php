...
/**
 * BK-108 — Host-profile route/controller positional scalar binding gate.
 *
 * Metadata-only: does not use RefreshDatabase (routes register at boot).
 */
#[Group('host-profile-contract')]
class TenantRouteControllerPositionalBindingGateTest extends TestCase
{
    ...
    #[Test]
    public function host_tenant_routes_do_not_expose_positional_scalar_injection(): void
    {
        $violations = $this->scanTenantAddressedControllerRoutes();
        $this->assertSame(
            [],
            $violations,
            "BK-108 positional binding violations (Host):\n".$this->formatBk108PositionalViolations($violations)
        );
    }
}
